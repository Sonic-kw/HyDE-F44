...
# grub2 (Fedora uses grub2-mkconfig and different cfg paths)
# Detect UEFI vs BIOS to determine the correct output path.
grub2_cfg=""
if [ -f /boot/efi/EFI/fedora/grub.cfg ]; then
    grub2_cfg="/boot/efi/EFI/fedora/grub.cfg"
elif [ -f /boot/grub2/grub.cfg ]; then
    grub2_cfg="/boot/grub2/grub.cfg"
fi

if pkg_installed grub2-common && [ -n "$grub2_cfg" ]; then
    echo -e "\033[0;32m[BOOTLOADER]\033[0m detected // grub2 (${grub2_cfg})"

    if [ ! -f /etc/default/grub.t2.bkp ] && [ ! -f "${grub2_cfg}.t2.bkp" ]; then
        echo -e "\033[0;32m[BOOTLOADER]\033[0m configuring grub2..."
        sudo cp /etc/default/grub /etc/default/grub.t2.bkp
        sudo cp "${grub2_cfg}" "${grub2_cfg}.t2.bkp"

        if nvidia_detect; then
            echo -e "\033[0;32m[BOOTLOADER]\033[0m nvidia detected, adding nvidia_drm.modeset=1 to boot option..."
            gcld=$(grep "^GRUB_CMDLINE_LINUX=" "/etc/default/grub" | cut -d'"' -f2 | sed 's/\b nvidia_drm.modeset=.\b//g')
            sudo sed -i "/^GRUB_CMDLINE_LINUX=/c\GRUB_CMDLINE_LINUX=\"${gcld} nvidia_drm.modeset=1\"" /etc/default/grub
        fi
        ...
        sudo grub2-mkconfig -o "${grub2_cfg}"
    else
        echo -e "\033[0;33m[SKIP]\033[0m grub2 is already configured..."
    fi
fi
